...
export default function AddCar({setVisible, refresh}:showPage) {
    ...
    const [gear, setGear] = useState<Gear>(Gear.Manual)
    const [energy, setEnergy] = useState<Energy>(Energy.Fuel)
    const [price, setPrice] = useState<number>()
    const [image, setImage] = useState<File>()

    async function addNewcar(e:React.FormEvent<HTMLFormElement>) {
        e.preventDefault()

        if(!brand || !model || !segment || !seats || !baggage || !door || !gear || !energy || !price || !image){
            ...
            try{
                ...
                await axios.post('/api/cars/new',data,{
                    headers: {
    ...
    return (
        <div className='absolute z-20 top-0 left-0 bg-[#0000006c] w-full h-screen flex justify-center items-center'>
            <form onSubmit={addNewcar} className='border-[1.5px] border-solid border-[#FFFFFF66] rounded-2xl backdrop-blur-xl px-10 pb-5 pt-6'>
                <h1 className='text-white font-bold text-2xl'>Add new car</h1>
                   ...
                   <div className='border-l border-solid border-[#6B7280] pl-4'>
                        <div className='grid grid-cols-[2fr_3fr] gap-3'>
                            <div>
                                ...
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="text" className='outline-none' onChange={(e) => setBrand(e.target.value)}/>
                                </div>
                            </div>
                            <div>
                                <p className='text-lg font-medium text-[#2F2F2F]'>Model Name</p>
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="text" className='outline-none' onChange={(e) => setModel(e.target.value)}/>
                                </div>
                            </div>
                        </div>
                        <div className='grid grid-cols-5 gap-3 pt-3'>
                            <div>
                                ...
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="number" className='outline-none w-full' onChange={(e) => setSeats(Number(e.target.value))}/>
                                </div>
                            </div>
                            <div>
                                <p className='text-lg font-medium text-[#2F2F2F]'>Backages</p>
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="number" className='outline-none w-full' onChange={(e) => setBaggage(Number(e.target.value))}/>
                                </div>
                            </div>
                            <div>
                                <p className='text-lg font-medium text-[#2F2F2F]'>Doors</p>
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="number" className='outline-none w-full' onChange={(e) => setDoor(Number(e.target.value))}/>
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div className=' border border-solid border-[#6B7280] rounded-lg w-full py-0.5 flex items-center px-2'>
                                    <input type="number" className='w-full outline-none' onChange={(e) => setPrice(Number(e.target.value))}/>
                                </div>
                            </div>
                            <div>
